...
l_p = tf.add(tf.matmul(Phip_x, W_p),b_p)
l_p_concat = tf.concat(1, [l_a, l_p])

# Fill best antecedent using max and all
f_x_ana = tf.add(tf.matmul(tf.nn.tanh(l_p_concat), u), b_u)
...
with tf.Session() as sess:
	...
	for i in range(TRAINING_SIZE):
		...
		for j in range(i):
			best_ment.append(sess.run(f_x_ana, feed_dict={Phia_x: mentionFeats[i].reshape(1,W2V_SIZE) ,Phip_x: getPairFeats(i, mentionFeats, W2V_SIZE)[j].shape([1, W2V_SIZE])}))	
		best_ment.append(sess.run(f_x_nonana, feed_dict={Phia_x: mentionFeats[i].reshape(1,W2V_SIZE)}))
	...
	for i in range(TRAINING_SIZE):
		...
		print(i, sess.run(f_x_best, feed_dict={Phia_x: mentionFeats[i].reshape(1,W2V_SIZE) ,Phip_x: getPairFeats(i, mentionFeats, W2V_SIZE) ,Y_antecedent: latent_antecedents}))
