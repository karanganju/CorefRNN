...
# conll_ments = [f for f in listdir(DATA_DIR) if isfile(join(DATA_DIR, f))]

opts, args = getopt.getopt(sys.argv[1:],"n:l:d:",[])
for opt, arg in opts:
	if opt == '-n':
...
print ITERATION_COUNT, LEARNING_RATE
...
with tf.Session() as sess:
	...
	cluster_data = getClustersArrayForMentions('mentionsList.txt')
	...
	for iteration_count in range(ITERATION_COUNT):
		...
		for i in range(TRAINING_SIZE):
			...
			ant = np.array(sess.run(best_ant, feed_dict={Phia_x: mentionFeats[i].reshape(1,W2V_SIZE) ,Phip_x: getPairFeats(i, mentionFeats, W2V_SIZE) ,Y_antecedent: latent_antecedents}))
